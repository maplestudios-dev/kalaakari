...
r.post('/', requireAuth, requirePermission('video.write'), asyncHandler(async (req, res) => {
  ...
  // Only one video may be the pinned /reel hero at a time (homepage `featured` may be many).
  if (item.reelHero) await Video.updateMany({ _id: { $ne: item._id } }, { $set: { reelHero: false } })
...
r.put('/:id', requireAuth, requirePermission('video.write'), asyncHandler(async (req, res) => {
  ...
  if (!item) { res.status(404); throw new Error('Not found') }
  // Pinning this one as the /reel hero unpins any other (homepage `featured` is unaffected).
  if (req.body.reelHero) await Video.updateMany({ _id: { $ne: item._id } }, { $set: { reelHero: false } })
