...
export default function Contact() {
  const c = useCopy('pages.contact') || {}
  const thanks = c.thanks || {}
          ...
          {done ? (
            <FadeContent>
              <div className="border border-saffron p-16 text-center">
                <div className="font-display text-saffron text-6xl">{thanks.title}</div>
                <p className="font-deva text-mustard mt-4 text-2xl">{thanks.deva}</p>
                <p className="font-serif-i text-ink-mute mt-6 text-lg max-w-xl mx-auto leading-relaxed">
                  {thanks.body}<a className="text-saffron underline" href={`mailto:${thanks.urgentEmail || 'hello@kalaakaari.in'}`}>{thanks.urgentEmail || 'hello@kalaakaari.in'}</a>.
                </p>
              </div>
            </FadeContent>
          ) : (
                ...
                <div className="md:col-span-2 flex flex-wrap items-center gap-4 pt-4">
                  <Magnet>
                    <button disabled={isSubmitting} className="inline-flex items-center gap-3 px-8 py-5 bg-saffron text-bg text-[12px] tracking-[.24em] uppercase hover:bg-mustard transition-colors disabled:opacity-50">
                      {isSubmitting ? 'Sending…' : (c.submitLabel || 'Send Brief →')}
                    </button>
                  </Magnet>
                  <span className="label-tag normal-case tracking-[.12em] text-ink-mute">{c.responseNote}</span>
                </div>
              </form>
            </FadeContent>
          )}
